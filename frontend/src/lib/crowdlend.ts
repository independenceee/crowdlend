...
// preprod testnet: slot ↔ POSIX ms (genesis = 1666656000 Unix s, 1 slot = 1 s)
const PREPROD_GENESIS_UNIX_S = 1666656000;
const slotToPosixMs = (slot: number) => (PREPROD_GENESIS_UNIX_S + slot) * 1000;
const posixMsToSlot = (posixMs: number) => Math.floor(posixMs / 1000) - PREPROD_GENESIS_UNIX_S;

const provider = new BlockfrostProvider(BLOCKFROST_API_KEY);

export const SCRIPT_ADDRESS = resolvePlutusScriptAddress({ code: LENDING_SCRIPT_CBOR, version: "V3" }, NETWORK_ID);
...
export async function fetchLoans(): Promise<LoanUtxo[]> {
    const url = `https://cardano-preprod.blockfrost.io/api/v0/addresses/${SCRIPT_ADDRESS}/utxos`;
...
export function getNftPolicyId(issuerPkh: string): {
    ...
    const appliedCbor = applyParamsToScript(NFT_POLICY_CBOR, [{ bytes: issuerPkh }], "JSON");
...
export async function createLoan(
    ...
    const nftUtxo = utxos.find((u: any) => u.output.amount.some((a: any) => a.unit === collateralUnit));
...
export async function fundLoan(wallet: any, loanUtxo: LoanUtxo): Promise<string> {
    ...
    const nowSlot = Number(resolveSlotNo("preprod", Date.now()));
...
function buildMeshDatum(loan: LoanDatum) {
    const statusConstructor =
        loan.status.type === "Pending" ? { constructor: 0, fields: [] } : { constructor: 1, fields: [{ int: (loan.status as any).funded_at }] };
    return {
        constructor: 0,
        fields: [
            { bytes: loan.borrower },
            loan.lender !== null ? { constructor: 0, fields: [{ bytes: loan.lender }] } : { constructor: 1, fields: [] },
            { int: loan.principal },
            { int: loan.interest_rate },
            { int: loan.loan_duration },
            loan.due_date !== null ? { constructor: 0, fields: [{ int: loan.due_date }] } : { constructor: 1, fields: [] },
...
function parseLoanDatum(raw: any): LoanDatum {
    ...
    // eslint-disable-next-line eqeqeq
    const status =
        statusRaw.constructor == 0 ? ({ type: "Pending" } as const) : ({ type: "Active", funded_at: Number(statusRaw.fields[0].int) } as const);
