    ...
    public convertDatum = ({
        ...
        try {
            ...
            const buildAddress = (paymentHex: string, stakeHex?: string): string => {
                if (!paymentHex || paymentHex.length !== 56) return "";
                return serializeAddressObj(pubKeyAddress(paymentHex, stakeHex || "", false), APP_NETWORK_ID);
            };

            const borrowerWrap = datum.fields?.[0]?.fields?.[0];
            const borrowerPayment = borrowerWrap?.fields?.[0]?.fields?.[0]?.bytes;
            const borrowerStake = borrowerWrap?.fields?.[1]?.fields?.[0]?.fields?.[0]?.fields?.[0]?.bytes;
            const borrower = buildAddress(borrowerPayment, borrowerStake);
